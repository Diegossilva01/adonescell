...
function usuarioSalvo(){try{return JSON.parse(localStorage.getItem('adones_user')||'null')}catch(_){return null}}
const state={token:localStorage.getItem('adones_token')||'',user:usuarioSalvo(),ordens:[],recibos:[],estoque:[],estoqueAtualizadoEm:0,gastos:[],retiradas:[],usuarios:[],status:[],retiradaStatus:[],receiptItems:[]};
const API_TIMEOUT_MS=30000;
const STOCK_LOCAL_KEY='adones_estoque_cache_v13';

function carregarEstoqueLocal(){
  try{
    const c=JSON.parse(localStorage.getItem(STOCK_LOCAL_KEY)||'null');
    if(c&&Array.isArray(c.produtos)){
      state.estoque=c.produtos;
      state.estoqueAtualizadoEm=Number(c.ts||0);
    }
  }catch(_){}
}
function salvarEstoqueLocal(){
  try{localStorage.setItem(STOCK_LOCAL_KEY,JSON.stringify({ts:Date.now(),produtos:state.estoque}))}catch(_){}
  state.estoqueAtualizadoEm=Date.now();
}
function resumoEstoqueLocal(){
  const ativos=state.estoque.filter(p=>p.status==='Ativo');
  return {
    produtosAtivos:ativos.length,
    unidades:ativos.reduce((s,p)=>s+Number(p.quantidade||0),0),
    custoEstoque:ativos.reduce((s,p)=>s+Number(p.quantidade||0)*Number(p.custo||0),0),
    potencialVenda:ativos.reduce((s,p)=>s+Number(p.quantidade||0)*Number(p.preco||0),0)
  };
}
function filtroEstoqueLocal(){
  const busca=String($('#buscaEstoque')?.value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
  const status=String($('#filtroEstoqueStatus')?.value||'').trim();
  return state.estoque.filter(p=>{
    const hay=[p.nome,p.sku,p.categoria].join(' ').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
    return (!busca||hay.includes(busca))&&(!status||p.status===status);
  });
}
function idProdutoCliente(){
  const rnd=(globalThis.crypto&&crypto.randomUUID)?crypto.randomUUID().replace(/-/g,'').slice(0,12):Math.random().toString(36).slice(2,14);
  return 'PROD-WEB-'+rnd.toUpperCase();
}
function erroTransitorio(e){return /falha de conexão|demorou para responder|resposta inválida|network|failed to fetch|load failed/i.test(String(e?.message||e||''))}
async function apiRetry(acao,dados={},tentativas=1){
  let ultimo;
  for(let i=0;i<=tentativas;i++){
    try{return await api(acao,dados)}catch(e){ultimo=e;if(i>=tentativas||!erroTransitorio(e))throw e;await esperar(650*(i+1))}
  }
  throw ultimo;
}
...
async function api(acao,dados={}){
  ...
  try{
    ...
    if(!j.ok){
      ...
      if(erroSessao(erro) && acao!=='login'){limparSessaoLocal();mostrarLogin('Sua sessão expirou. Entre novamente.');}
      throw erro;
    }
    return j;
  }catch(err){
...
function erroSessao(err){return /sessão expirada|sessao expirada/i.test(String(err?.message||''))}
function salvarSessaoLocal(){
  if(state.token)localStorage.setItem('adones_token',state.token);
  if(state.user)localStorage.setItem('adones_user',JSON.stringify(state.user));
}
function limparSessaoLocal(){
  state.token='';state.user=null;
  localStorage.removeItem('adones_token');
  localStorage.removeItem('adones_user');
}
...
$('#loginForm').onsubmit=async e=>{
  ...
  try{const j=await api('login',d);state.token=j.token;state.user=j.usuario;salvarSessaoLocal();$('#loginMsg').textContent='';showApp()}catch(err){$('#loginMsg').textContent=err.message}
};
$('#sair').onclick=async()=>{try{if(state.token)await api('logout')}catch(_){ }limparSessaoLocal();localStorage.removeItem('adones_page');location.reload()};
...
async function alterarStatusRapido(select){
  ...
  try{
    ...
    for(let tentativa=0;tentativa<2;tentativa++){
      try{resposta=await api('atualizarStatusOrdem',{id,status:novo});break}
      catch(e){
        ultimoErro=e;
        if(erroSessao(e))throw e;
        const transitorio=/falha de conexão|demorou para responder|resposta inválida/i.test(String(e.message||''));
        if(!transitorio||tentativa===1)throw e;
        saving.textContent='Tentando novamente...';
        await esperar(900);
      }
    }
    if(!resposta)throw ultimoErro||new Error('Não foi possível atualizar o status.');
    ...
    saving.textContent='✓ Salvo';toast(`Status alterado para ${novo}`);
    setTimeout(()=>{saving.textContent='';carregarOrdens()},900);
  }catch(e){
    select.value=anterior;
    saving.textContent='Não salvo';
    toast(e.message);
    setTimeout(()=>saving.textContent='',2200);
...
async function carregarEstoqueParaVenda(){
  // Mostra o cache imediatamente; sincroniza em segundo plano quando necessário.
  if(state.estoque.length) renderItensRecibo();
  const fresco=Date.now()-Number(state.estoqueAtualizadoEm||0)<90000;
  if(fresco) return;
  try{
    const j=await api('listarEstoque',{});
    state.estoque=j.produtos||[];
    salvarEstoqueLocal();
    renderItensRecibo();
  }catch(e){ if(!state.estoque.length) toast(e.message); }
}
...
async function carregarEstoque({silencioso=false,forcar=false}={}){
  // Render instantâneo com os dados já guardados no aparelho.
  if(state.estoque.length) renderEstoque(resumoEstoqueLocal(),filtroEstoqueLocal());
  const fresco=Date.now()-Number(state.estoqueAtualizadoEm||0)<60000;
  if(fresco&&!forcar) return;
  try{
    const j=await api('listarEstoque',{});
    state.estoque=j.produtos||[];
    salvarEstoqueLocal();
    renderEstoque(j.resumo||resumoEstoqueLocal(),filtroEstoqueLocal());
  }catch(e){
    if(!silencioso&&!state.estoque.length) toast(e.message);
  }
}

function renderEstoque(resumo=resumoEstoqueLocal(),lista=filtroEstoqueLocal()){
  ...
  body.innerHTML=lista.length?lista.map(p=>`<tr>
...
$('#reciboForm').onsubmit=async e=>{e.preventDefault();atualizarTotalRecibo();const d=formObj(e.currentTarget);d.itens=JSON.stringify(itensPayload());const janela=window.open('','_blank');if(janela)janela.document.write('<!doctype html><title>Gerando recibo...</title><p style="font-family:Arial;padding:30px">Gerando recibo...</p>');$('#reciboMsg').textContent='Salvando venda...';try{const j=await api('salvarRecibo',d);$('#reciboMsg').textContent=`Venda registrada: ${j.numero}`;if(j.recibo)imprimirRecibo(j.recibo,janela);else if(janela)janela.close();toast('Venda registrada e estoque atualizado');
  // Atualiza o estoque local imediatamente, sem esperar outra ida ao servidor.
  state.receiptItems.forEach(it=>{if(it.tipo==='estoque'&&it.produtoId){const p=state.estoque.find(x=>x.id===it.produtoId);if(p)p.quantidade=Math.max(0,Number(p.quantidade||0)-Math.max(1,parseInt(it.quantidade,10)||1))}});
  salvarEstoqueLocal();renderItensRecibo();
  carregarRecibos();setTimeout(()=>carregarEstoque({silencioso:true,forcar:true}),300);setTimeout(limparFormRecibo,500)}catch(err){if(janela)janela.close();$('#reciboMsg').textContent=err.message}};
...
$('#buscarEstoque').onclick=()=>renderEstoque(resumoEstoqueLocal(),filtroEstoqueLocal());
$('#buscaEstoque').addEventListener('input',()=>renderEstoque(resumoEstoqueLocal(),filtroEstoqueLocal()));
$('#buscaEstoque').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();renderEstoque(resumoEstoqueLocal(),filtroEstoqueLocal())}});
$('#filtroEstoqueStatus').onchange=()=>renderEstoque(resumoEstoqueLocal(),filtroEstoqueLocal());
$('#produtoForm').onsubmit=async e=>{
  e.preventDefault();
  const f=e.currentTarget;
  const d=formObj(f);
  const snapshot=state.estoque.map(x=>({...x}));
  const existente=String(d.id||'').trim();
  if(!existente)d.clientId=idProdutoCliente();
  const pid=existente||d.clientId;
  const otimista={
    id:pid,nome:String(d.nome||'').trim(),sku:String(d.sku||'').trim(),categoria:String(d.categoria||'').trim(),
    quantidade:Math.max(0,parseInt(d.quantidade,10)||0),estoqueMinimo:Math.max(0,parseInt(d.estoqueMinimo,10)||0),
    custo:num(d.custo),preco:num(d.preco),status:d.status==='Inativo'?'Inativo':'Ativo',atualizado:'Sincronizando…'
  };
  const pos=state.estoque.findIndex(x=>x.id===pid);
  if(pos>=0)state.estoque[pos]={...state.estoque[pos],...otimista};else state.estoque.push(otimista);
  state.estoque.sort((a,b)=>String(a.nome).localeCompare(String(b.nome),'pt-BR'));
  salvarEstoqueLocal();
  renderEstoque(resumoEstoqueLocal(),filtroEstoqueLocal());
  $('#produtoModal').classList.add('hidden');
  toast('Produto adicionado. Sincronizando com o Google…');
  try{
    const j=await apiRetry('salvarProduto',d,1);
    if(j.produto){
      const i=state.estoque.findIndex(x=>x.id===pid||x.id===j.id);
      if(i>=0)state.estoque[i]=j.produto;else state.estoque.push(j.produto);
    }
    salvarEstoqueLocal();
    renderEstoque(resumoEstoqueLocal(),filtroEstoqueLocal());
    $('#produtoMsg').textContent='';
    toast('Produto salvo no Google.');
  }catch(err){
    state.estoque=snapshot;
    salvarEstoqueLocal();
    renderEstoque(resumoEstoqueLocal(),filtroEstoqueLocal());
    $('#produtoModal').classList.remove('hidden');
    $('#produtoMsg').textContent=err.message;
    toast('Não foi possível sincronizar o produto.');
  }
};
...
(async()=>{
  ...
  // Se já temos usuário salvo, abre o painel imediatamente e valida em segundo plano.
  // Assim um refresh não parece um logout enquanto o Apps Script está iniciando.
  if(state.user)showApp();
  else $('#loginMsg').textContent='Restaurando sua sessão...';

  let ultimoErro=null;
  for(let tentativa=0;tentativa<3;tentativa++){
    try{
      const j=await api('verificarToken');
      state.user=j.usuario;
      salvarSessaoLocal();
      if($('#app').classList.contains('hidden'))showApp();
      return;
    }catch(e){
      ultimoErro=e;
      if(erroSessao(e)){limparSessaoLocal();mostrarLogin('Sua sessão expirou. Entre novamente.');return}
      if(tentativa<2)await esperar(800*(tentativa+1));
    }
  }

  // Falha de internet/Google não apaga a sessão local. O usuário não precisa logar de novo.
  if(state.user){toast('Conexão com o Google instável. Sua sessão foi mantida.');}
  else{$('#loginMsg').textContent='Não foi possível conectar agora. Recarregue a página em instantes.';}
})();
